...
function Biodata() {
  ...
  const relawanRuslan_userData = JSON.parse(localStorage.getItem('relawanRuslan_userData'));
  const relawanRuslan_id = (localStorage.getItem('relawanRuslan_id'));
  console.log('cek id user: ', relawanRuslan_id)
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        ...
        // Filter the data array based on relawanRuslan_id
        const filteredData = sanityData.filter(item => item._id === relawanRuslan_id);
  ...
  const updateSanityUser = async (relawanRuslan_userData) => {
    try {
      const response = await fetch(`https://ln9ujpru.api.sanity.io/v2021-03-25/data/mutate/production`, {
        ...
        body: JSON.stringify({
          mutations: [
            {
              patch: {
                id: relawanRuslan_id, // The _id of the document to update
                set: {
                  umur: relawanRuslan_userData.umur,
                  gender: relawanRuslan_userData.gender,
                  alamat: relawanRuslan_userData.alamat,
                  faskes: relawanRuslan_userData.faskes,
                  tb: relawanRuslan_userData.tb,
                  bb: relawanRuslan_userData.bb,
                  keluhan: relawanRuslan_userData.keluhan,
                  telepon: relawanRuslan_userData.telepon,
                },
              },
            },
          ],
        ...
        <div className="lg:flex">
          ...
          <div className="lg:w-1/2 lg:py-0 py-10 flex items-center">
            <form className="w-full lg:px-28 px-4 items-center" onSubmit={handleSubmit}>
              <h2 className="font-bold text-[18px] lg:text-4xl mb-10 text-gray-900 uppercase">Biodata <span className="text-sm">({relawanRuslan_userData[0]?.name})</span></h2>
              <Input
                type="text"
                name="umur"
                placeholder="Umur"
